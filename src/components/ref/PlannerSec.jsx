...
export function PlannerSec({ global, saveGlobal }) {
  ...
  return (<div><Title>Pla de 4 setmanes</Title><p style={{ fontSize: 11, color: "#8a7f74", marginBottom: 14 }}>Planifica sessions. Prem un dia per afegir.</p>
    {weeks.map((wd, wi) => { const ws = new Date(wd[0] + "T12:00"), we = new Date(wd[6] + "T12:00"); return (
      <div key={wi} style={{ marginBottom: 16 }}>
        <div style={{ fontSize: 11, fontWeight: 600, color: "#8a7f74", marginBottom: 6 }}>Setmana {wi + 1} · {ws.getDate()}/{ws.getMonth()+1} - {we.getDate()}/{we.getMonth()+1}</div>
        <div style={{ display: "flex", gap: 3 }}>
          {wd.map((dk, di) => { const items = plan.filter(p => p.date === dk); const isT = dk === todayKey(); const past = dk < todayKey(); return (
            <button key={dk} onClick={() => setSelDate(selDate === dk ? null : dk)} style={{ flex: 1, padding: "4px 2px", borderRadius: 6, border: isT ? "2px solid #c4855a" : selDate === dk ? "2px solid #60a5fa" : "1px solid #ede8e3", background: past ? "#faf7f4" : "#ffffff", cursor: "pointer", textAlign: "center", opacity: past ? 0.5 : 1, minHeight: 52 }}>
              <div style={{ fontSize: 8, color: isT ? "#c4855a" : "#8a7f74", fontWeight: isT ? 700 : 400 }}>{dn[di]}</div>
              <div style={{ fontSize: 10, color: "#8a7f74", marginBottom: 2 }}>{new Date(dk + "T12:00").getDate()}</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 1, alignItems: "center" }}>{items.map(it => { const tp = TRAIN_TYPES_PLAN.find(t => t.id === it.type); return <div key={it.id} style={{ width: 8, height: 8, borderRadius: 2, background: tp?.color || "#b6aa9e" }} />; })}</div>
            </button>); })}
        </div>
