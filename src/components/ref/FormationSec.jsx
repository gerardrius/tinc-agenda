...
export function FormationSec({ day, u, global, saveGlobal }) {
  ...
  return (<div><Title>Formació</Title>
    <p style={{ fontSize: 11, color: "#6b7280", marginBottom: 14 }}>Pla recurrent setmanal — es repeteix cada setmana. Prem un dia per editar-lo.</p>

    <Card>
      <Lbl>Avui — {WEEKDAYS[todayDow]}</Lbl>
      {todayItems.length === 0 && <p style={S.muted}>Cap ítem de formació per avui.</p>}
      {todayItems.map(it => (
        <button key={it.id} onClick={() => toggleToday(it.id)} style={{ ...S.habBtn, background: day.formation?.[it.id] ? "rgba(74,222,128,0.07)" : "transparent", borderColor: day.formation?.[it.id] ? "rgba(74,222,128,0.22)" : "#252a30" }}>
          <span style={{ ...S.chk, background: day.formation?.[it.id] ? "#4ade80" : "#252a30", color: day.formation?.[it.id] ? "#000" : "#444" }}>{day.formation?.[it.id] ? "✓" : ""}</span>
          <span style={{ fontSize: 12, color: day.formation?.[it.id] ? "#a7f3d0" : "#888" }}>{it.label}</span>
        </button>
      ))}
    </Card>
    ...
    <div style={{ display: "flex", gap: 3, marginBottom: 6 }}>
      {WEEKDAYS.map((wd, di) => { const items = plan.filter(p => p.dow === di); const isT = di === todayDow; return (
        <button key={di} onClick={() => setSelDow(selDow === di ? null : di)} style={{ flex: 1, padding: "4px 2px", borderRadius: 6, border: isT ? "2px solid #4ade80" : selDow === di ? "2px solid #a78bfa" : "1px solid #222830", background: "#141719", cursor: "pointer", textAlign: "center", minHeight: 52 }}>
          <div style={{ fontSize: 8, color: isT ? "#4ade80" : "#6b7280", fontWeight: isT ? 700 : 400 }}>{wd.slice(0, 2)}</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 1, alignItems: "center", marginTop: 4 }}>{items.map(it => { const tp = FORMATION_TYPES.find(t => t.id === it.type); return <div key={it.id} style={{ width: 8, height: 8, borderRadius: 2, background: tp?.color || "#555" }} />; })}</div>
        </button>); })}
    </div>
