...
export function computeDomainStats({ id, global, allData, garminSleep, domainScores }) {
  ...
  if (id === "relacions") {
    const counts = dates.map((dk) => (allData[dk]?.social || []).length);
    ...
    return {
      ...
      kv: [["Entrades registrades", String(total)]],
      insight: total === 0
  ...
  if (id === "salut") {
    ...
    const counts = dates.map((dk) => salutHabits.filter((h) => allData[dk]?.habits?.[h.id]).length);
    const total = counts.reduce((a, b) => a + b, 0);
    const possible = salutHabits.length * 7;
    return {
      emoji: "🏃", title: "Salut", color: COLORS.domainSalut,
      value: possible ? Math.round((total / possible) * 10) : "—", qualifier: "de 10, hàbits complerts",
      bars: counts.map((v, i) => ({ v, label: WEEKDAYS_ABBR[i] })),
      kv: [["Hàbits complerts", `${total}/${possible}`]],
  ...
  if (id === "feina") {
    let done = 0, tot = 0;
    dates.forEach((dk) => (allData[dk]?.tasks || []).forEach((t) => { if (t.topic === "feina") { tot++; if (t.done) done++; } }));
    return {
      emoji: "💼", title: "Feina", color: COLORS.domainFeina,
      value: tot ? `${done}/${tot}` : "—", qualifier: "tasques tancades",
      bars: dates.map((dk, i) => ({ v: (allData[dk]?.tasks || []).filter((t) => t.topic === "feina" && t.done).length, label: WEEKDAYS_ABBR[i] })),
  ...
  return {
    ...
    kv: [["Preparació", upcoming ? `${Object.values(upcoming.prep || {}).filter(Boolean).length}/${Object.keys(upcoming.prep || {}).length || "—"}` : "Cap partit actiu"]],
